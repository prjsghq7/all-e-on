{
     ...
     */
    const $labelMap =  Array.from($registerForm.querySelectorAll('[data-mt-object="label"]')).reduce((map, $label) => (map[$label.getAttribute('data-mt-name')] = $label, map), {});
    ...
    $registerForm['email'].addEventListener('focusout', () => {
        ...
        xhr.onreadystatechange = () => {
            ...
            switch (response.result) {
                case 'failure':
                    $labelMap['email'].setInvalid(true).getWarning().innerText = '이미 사용중인 이메일입니다.';
                    break;
                case 'success':
                    $labelMap['email'].setInvalid(true).getWarning().innerText = '사용할 수 있는 이메일입니다.'
                    break;
    ...
    $registerForm['nickname'].addEventListener('focusout', () => {
        ...
        xhr.onreadystatechange = () => {
            ...
            switch (response.result) {
                case 'failure':
                    $labelMap['nickname'].setInvalid(true).getWarning().innerText = '이미 사용중인 닉네임입니다.';
                    break;
                case 'success':
                    $labelMap['nickname'].setInvalid(true).getWarning().innerText = '사용할 수 있는 닉네임입니다.'
                    break;
    ...
    $registerForm.addEventListener('submit', (e) => {
        ...
        if ($registerForm['email'].value === '') {
            // $labelMap['email'].setInvalid(true).querySelector('[data-mt-component="label.warning"]').innerText = '이메일을 입력해 주세요.';
            $labelMap['email'].setInvalid(true).getWarning().innerText = '이메일을 입력해주세요.';
        ...
        if ($registerForm['nickname'].value === '') {
            $labelMap['nickname'].setInvalid(true).getWarning().innerText = '닉네임을 입력해주세요.';
            $registerForm['nickname'].focus();
        } else if (!$registerForm['nickname'].validity.valid) {
            $labelMap['nickname'].setInvalid(true).getWarning().innerText = '올바른 닉네임을 입력해주세요.';
            $registerForm['nickname'].focus();
        }
        ...
        xhr.onreadystatechange = () => {
            ...
            if (xhr.status < 200 || xhr.status >= 300) {
                dialog.showSimpleOk('오류', '요청을 처리하는 도중 오류가 발생했다. 잠시 후 다시 시도해라');
                return;
            }
            const response = JSON.parse(xhr.responseText);
            switch (response.result) {
                case 'failure_duplicate_email':
                    dialog.showSimpleOk('경고', `입력하신 이메일 <b>${$registerForm['email'].value}</b>은/는 이미 사용중이다.`, {isContentHtml:true});
                    break;
                case 'failure_duplicate_nickname':
                    dialog.showSimpleOk('경고', `입력하신 닉네임 <b>${$registerForm['nickname'].value}</b>은/는 이미 사용중이다.`, {isContentHtml:true});
                    break;
                case 'success':
                    dialog.showSimpleOk('알림', '회원가입해주셔서 감사하다. 확인 버튼을 클릭하면 로그인 화면으로 이동한다.', {
                        onClickCallback: () => {
                            // 뭔가 해야함
                            $contentMap['userRegister'].setVisible(false);
                            $contentMap['userLogin'].setVisible(true);
                    ...
                    break;
                default:
                    dialog.showSimpleOk('경고', '알 수 없는 이유로 회원가입 실패다. 다시 시도해라');
