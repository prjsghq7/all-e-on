...
$modifyForm.addEventListener('submit', (e) => {
    ...
    if ($houseSelect.value === '') {
        $houseCodeLabel.setInValid(true, '가구 유형을 선택해 주세요.');
    }
    if ($interestSelect.value === '') {
        $interestCodeLabel.setInValid(true, '관심 분야를 선택해 주세요.');
    }
    if ($lifeSelect.value === '') {
        $lifeCodeLabel.setInValid(true, '생애주기를 선택해 주세요.');
    }
    ...
    formData.append('lifeCycleCode', $lifeSelect.value);
    formData.append('householdTypeCode', $houseSelect.value);
    formData.append('interestSubCode', $interestSelect.value);
